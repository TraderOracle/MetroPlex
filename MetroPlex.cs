...
namespace MetroPlex
{
    public class MetroPlex : ChartStrategy
    {
        ...
        protected override void OnCalculate(int bar, decimal value)
        {
            ...
            var stUp = ((ValueDataSeries)_st.DataSeries[0])[pbar];
            var stDown = ((ValueDataSeries)_st.DataSeries[1])[pbar];
            ...
            if (fisherUp && hullUp && stUp > 0 && t1 > 0)
                OpenPosition("Standard Buy Signal", candle, bar, 1);

            if (fisherDown && hullDown && stDown > 0 && t1 < 0)
        ...
        private void OpenPosition(String sReason, IndicatorCandle c, int bar, int iDirection = -1)
        {
            ...
            if (Math.Abs(CurrentPosition) >= iAdvMaxContracts)
                return;
